(function(P){
'use strict';
const C=P.CampaignAutoplayRC39V005;if(typeof C!=='function')return;
...
proto.update=function(t){
 ...
 // Keep both validated moving attacks exercised in safe ground before combat pressure.
 ...
 // Pits keep the proven RC37 timing.
 ...
 // Hazard timing v006: take off close to the obstacle so the apex is over it; double-jump only while crossing on descent.
 ...
 for(let idx=0;idx<s.hazards.length;idx++){
   ...
   const d=h.left-playerRight;if(d>-45&&d<190){activeHazard={h,idx,d};break;}
 }
 if(activeHazard){const {h,idx,d}=activeHazard;
   if(grounded&&d<=88&&d>=52&&!jumpPlanned){if(this.pulseJump(t,'hazard_'+idx+'_late_takeoff',320)){jumpPlanned=true;axis=.84;}}
   const crossing=x>=h.left-12&&x<=h.right+18;
   if(!grounded&&p.airJumpsRemaining>0&&crossing&&vy>0&&!jumpPlanned){if(this.pulseJump(t,'hazard_'+idx+'_apex_extend',285)){jumpPlanned=true;}}
   if(crossing)axis=Math.min(axis,.72);else if(d<120)axis=Math.min(axis,.84);
 }

 // Authored jump cues remain normal-input navigation aids.
 ...
 // Combat v006: do not run into grounded enemies. Keep facing, hold position at weapon distance, and attack repeatedly until clear.
 ...
 if(nearest){const {e,idx,d}=nearest,ad=Math.abs(d);
   if(e.flying){if(d>0&&d<155){axis=Math.min(axis,.45);this.pulseAttack(t,'flying_'+idx,105);if(grounded&&!jumpPlanned){if(this.pulseJump(t,'flying_jump_'+idx,265))jumpPlanned=true;}}}
   else if(d>=0){
     if(d<150){axis=d>105?.36:d>78?.12:0;this.pulseAttack(t,'ground_hold_'+idx,110);}
     if(d<62&&!jumpPlanned&&grounded){if(this.pulseJump(t,'ground_emergency_vault_'+idx,275))jumpPlanned=true;}
   }else if(ad<38){axis=0;this.pulseAttack(t,'ground_overlap_'+idx,110);}
 }

 // If an arena gate is close and its enemies remain, refuse forward pressure until they are defeated.
 const gate=this.closedGateNear(x);if(gate){const alive=this.aliveForGate(gate);if(alive.length){axis=Math.min(axis,.18);}else axis=.22;}

 if(t-this.lastProgressAt>2300&&!jumpPlanned){if(this.pulseJump(t,'stuck_recovery',250)){this.recoveries++;this.lastProgressAt=t;}}
...
proto.snapshot=function(){const s=oldSnapshot.call(this);s.version='RC39_V006_INPUT_ONLY';return s;};
P.CampaignAutoplayRC39V006=C;
window.__KELVOR_RC39_V006_AUTOPLAY_PATCH_READY__=true;
})(PlatformerSNESV04);
