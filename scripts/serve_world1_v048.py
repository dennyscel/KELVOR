#!/usr/bin/env python3
"""Serve only the verified local v048 candidate, on 127.0.0.1:8770 by default."""
...
from build_world1_v048 import BUILD_ID, REPORT_NAME, BuildError, inventory, verify_payload
from serve_menu_v047 import LocalServer
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--root", type=Path, default=repo.parent.parent / "03_JOGO_LOCAL" / "CANDIDATO_V048")
    ...
    try:
        if not (root / "index.html").is_file() or not (root / REPORT_NAME).is_file():
            raise BuildError(f"Candidato não construído: {root}. Execute scripts/build_world1_v048.py primeiro.")
    ...
    print(f"KELVOR v048: {url}", flush=True)
