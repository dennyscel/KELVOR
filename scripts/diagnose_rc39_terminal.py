#!/usr/bin/env python3
"""Fast terminal-state diagnostic for RC39 campaign autoplay.

Diagnostic-only: never changes game code, physics, lives, timers, or acceptance gates.
Stops collection early only after a terminal GAME OVER / FAIL / goal result is observed.
"""
...
def main() -> int:
    ...
    try:
        ...
        for i in range(max(1,args.seconds)):
            ...
            if posts:
                terminal_reason='post_received'; break
            qa=snap.get('qa') or {}; scene=snap.get('scene') or {}
            if scene.get('lifeCycle')=='gameover' or qa.get('status')=='FAIL':
                terminal_reason='gameover_or_fail'; break
            if scene.get('goal') or qa.get('goalReached') or qa.get('status')=='PASS':
                terminal_reason='goal_or_pass';
                # Give the native Gauntlet up to two seconds to POST its own result.
                for _ in range(2):
                    if posts: break
                    time.sleep(1)
                break
        ...
        summary={'url':url,'seconds':round(time.monotonic()-start,2),'post_received':bool(posts),'terminal_reason':terminal_reason,'last_snapshot':snapshots[-1] if snapshots else None,'browser_log_count':len(logs),'severe':[x for x in logs if x.get('level')=='SEVERE']}
